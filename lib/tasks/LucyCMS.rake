...
namespace :LucyCMS do
  
  # Example use:
  #   rake LucyCMS:import:all FROM=mysite.local TO=mysite.com SEED_PATH=/path/to/seed_data
  namespace :import do
    ...
    task :check_for_requirements => :environment do |task, args|
      ...
      @seed_path  = 
        LucyCMS.config.seed_data_path = 
        (args[:seed_path].present?? args[:seed_path] : nil) || LucyCMS.config.seed_data_path
  ...
  # Example use:
  #   rake LucyCMS:import:all FROM=mysite.com TO=mysite.local SEED_PATH=/path/to/seed_data
  namespace :export do
